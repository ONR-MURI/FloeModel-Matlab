function [latout,lonout,cerr,epstol] = reduceVerts(latin,lonin,epstol)
%REDUCEM  Reduce density of points in vector data
%
%  [LATOUT,LONOUT] = REDUCEM(LATIN,LONIN) reduces the number of points in
%  vector map data using the Douglas-Peucker line simplification algorithm.
%  This method recursively subdivides a polygon until a run of points can
%  be replaced by a straight line segment, with no point in that run
%  deviating from the straight line by more than the tolerance.  In this
%  case the tolerance is computed automatically.
%
%  [LATOUT,LONOUT] = REDUCEM(LATIN,LONIN,TOL) uses the provided tolerance.
%  The units of the tolerance are degrees of arc on the surface of a
%  sphere.
%
%  [LATOUT,LONOUT,CERR] = REDUCEM(...)  also returns a measure of the error
%  introduced by the simplification.  CERR is the difference in the arc
%  length of the original and reduced data, normalized by the original
%  length.
%
%  [LATOUT,LONOUT,CERR,TOL] = REDUCEM(...)  also returns the tolerance used
%  in the reduction. Useful when the tolerance is computed automatically.
%
%  See also: GEORESIZE, INTERPM.
...
% determine start and finish indices and implement D-P function
...
grs80InKilometers = referenceEllipsoid('grs80','kilometers');
ntot = length(lat);
rng = distance('rh',lat(1:ntot-1),lon(1:ntot-1),...
					lat(2:ntot),lon(2:ntot),grs80InKilometers);
c = sum(rng(~isnan(rng)));
