function [floe, fracture,FxOA,FyOA] =calc_trajectory(dt,ocean,winds,floe,HFo, doInt,c2_boundary,ubound,ucell)
...
if  floe.rmax+abs(floe.Xi)>max(c2_boundary(1,:))
    force_stick = 0;%0.01*ubound/dt*sign(floe.Xi)*(abs(floe.Xi)-max(c2_boundary(1,:)))*floe.mass/max(c2_boundary(1,:));
...
fc=0;%ocean.fCoriolis; %coriolis parameter
...
%             du=Uocn_interp-floe.Ui; dv=Vocn_interp-floe.Vi; ocean.turn_angle = 0;
...
%     save('floecalc.mat','floe')
if isnan(floe.Ui) || isnan(floe.ksi_ice) || isnan(floe.Vi) || isinf(floe.ksi_ice)
    xx =1 ;
    xx(1) = [1 2];
end

% if floe.alive == 0 && floe.mass > 100
%     xx = 1;
%     xx(1) =[1 2];
% end
floe.Fx = ext_force(1); 
