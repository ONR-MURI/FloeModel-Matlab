...
for ii = 1:length(R)
    ...
    Floes(ii).Fx = Floes(ii).mass*(floe1.Fx*floe1.mass + sum(cat(1,floe2.Fx).*mass))/(mtot^2);
    Floes(ii).Fy = Floes(ii).mass*(floe1.Fy*floe1.mass + sum(cat(1,floe2.Fy).*mass))/(mtot^2);
    Floes(ii).FxOA = [];
    Floes(ii).FyOA = [];
