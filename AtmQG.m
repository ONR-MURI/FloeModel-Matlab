function [psi12,q,qp] = AtmQG(c, pvold,iter_num,Nt,N,dt)
%% Two-layer QG model for atmosphere dynamics
...
taub = 1e-5;         % 1/tau_b, relaxation time scale for buoyancy flux, 10 days
...
% Diagnostics
countDiag = 1; % was 100, Compute diagnostics every countDiag steps
...
for ii=1:Nt
    ...
    if mod(ii,countDiag)==0
        ...
        if mod(ii, 1000)==0
            display(['iteration i = ', num2str(iter_num), '; time step dt = ',num2str(dt), ', ene = ',num2str(sum(KE+APE))]);
        end
