...
for kk=1:length(Floe)
    ...
    [V,D] = eig([SigXX(jj,ii) SigYX(jj,ii);SigXY(jj,ii) SigYY(jj,ii)]);
    %r = randi([1 2],1,1);
    [~,r] = max(diag(D));
    ...
    thet = atan(m)+(-1)^randi([0 1],1,1)*15*pi/180;
    ...
    if sqrt((x2-x1).^2-(y2-y1).^2)>10
        ...
        for p=1:length(a)
            ...
            if ~isempty(FloeNEW) && isfield(FloeNEW,'poly')
                FloeNEW=rmfield(FloeNEW,{'poly'});
            end
            
            Floes = [Floes FloeNEW];
            
            if abs(FloeNEW.ksi_ice) > 1e-4
                xx = 1; xx(1) =[1 2];
            end
...
if ~isempty(Floes) && isfield(Floes,'poly')
