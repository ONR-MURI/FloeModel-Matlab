function [ force_1, pcontact, overlap,Force_factor] = floe_interactions_polyc(floe1, floe2, c2_boundary,PERIODIC,Modulus)
...
%     [P, worked] = sort_contact_points(P, c1, c2 );
    r = regions(polyI);
    Amin =  min([area(poly1) area(poly2)]);
%         xx = 1; xx(1) =[1 2];
    Amin = 1.8e-4*Amin;
...
%     Force_factor = Force_factor*N_contact;
    ...
    for k=1:N_contact
        poly = r(k);
%         [d_min1] = p_poly_dist(P(1,:)', P(2,:)', poly.Vertices(:,1), poly.Vertices(:,2));
%         [~,I] = mink(d_min1,2);
...
%         force_dir=f_dir/sqrt(f_dir'*f_dir); %always push the flow away from contact point!
        ...
        dir_t = (v1-v2)/vecnorm(v1-v2);%[-force_dir(2) force_dir(1)];
        v_t = dot(v1-v2,dir_t);
        v_r = (v1+v2)/2;
        dl = mean(b);
        %                 force_t = dl*G*v_t*sign(v_r-v1).*abs(dir_t);
        %                 force_t = dl*G*v_t*dir_t;
        force_t = dl*G*v_t*dir_t;%[0 0];%-area(r(k))*(v1-v2)*Force_factor/4;%[0 0];%
