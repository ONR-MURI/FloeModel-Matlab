...
for ii = 1:Nx
    for jj = 1:Ny
        ...
        if Mtot>0
            ...
            eularian_data.force_x(jj,ii) = sum(ForceX(live == 1 & Binx == ii & Biny == jj).*mass(live == 1 & Binx == ii & Biny == jj))./Mtot;%sum(mass(logical(potentialInteractions(jj,ii,:)))'.*dU(logical(potentialInteractions(jj,ii,:)))'.*Aover)./(dt*Area);
