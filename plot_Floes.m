...
xmin=nanmin(c2_boundary(1,:));
xmax=nanmax(c2_boundary(1,:));

ymin=nanmin(c2_boundary(2,:));
ymax=nanmax(c2_boundary(2,:));

aspect_ratio=(xmax-xmin)/(ymax-ymin);
if (fig==0 || ~isvalid(fig))
    fig=figure('Position',[100 100 500*aspect_ratio 500],'visible','on');  
    set(fig,'PaperSize',8*[aspect_ratio 1],'PaperPosition',8*[0 0 aspect_ratio 1]);
end
...
hold on;
%axis([-1 1 -1 1]*7e4);
axis([xmin xmax ymin ymax]); %plot only up to boundaries.
...
for j=1:length(Floe)
    if Floe(j).alive
         ...
         ind=~isnan(c(1,:));
         fill(c(1,ind),c(2,ind),[1 1 1.2]*0.6);
         axis ij
    end
end
